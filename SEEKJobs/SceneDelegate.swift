...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let token = try? KeychainHelper().getToken(identifier: currentUser), !token.isEmpty {
            setRootVC(storyboardName: SharedConstants.Storyboard.main)
        }
        else {
            setRootVC(storyboardName: SharedConstants.Storyboard.login)
        }
        
        // Observers
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(willSetRootVC(_:)),
    ...
    @objc private func willSetRootVC(_ notification: Notification) {
